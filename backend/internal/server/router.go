...
func NewRouter(cfg *config.Config, db *gorm.DB) *gin.Engine {
	...
	api := r.Group("/api", middleware.MaxBodyBytes(1<<20))
	{
		...
		// Registration applies its own persistent limits in AuthHandler so a
		// restart cannot reset counters and threshold hits can create bans.
		...
		)
		{
			...
			admin.POST("/errors/site/resolve-batch", adminH.ResolveSiteErrorsBatch)
			admin.POST("/errors/site/:id/resolve", adminH.ResolveSiteError)
			admin.GET("/errors/api", adminH.OpsErrors)
			admin.POST("/errors/api/resolve-batch", adminH.ResolveAPIErrorsBatch)
