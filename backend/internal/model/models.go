...
// RegistrationRiskEvent is the durable input for registration anti-abuse
// decisions. It stores no email address or raw user-agent, only the minimum
// network/domain metadata needed for rolling counters.
type RegistrationRiskEvent struct {
	ID                  int64     `gorm:"primaryKey" json:"id"`
