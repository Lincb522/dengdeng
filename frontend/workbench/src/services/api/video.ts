...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], options?: RequestOptions): Promise<VideoGenerationTask> {
    const guidedConfig = applyCreationGuidance(config, "video");
    const selectedModel = (guidedConfig.model || guidedConfig.videoModel).trim();
    const requestConfig = resolveModelRequestConfig(guidedConfig, selectedModel);
    const guidedPrompt = requestConfig.systemPrompt.trim() ? `${requestConfig.systemPrompt.trim()}\n\n${prompt}` : prompt;
    const script = resolveModelScript(guidedConfig, selectedModel);
    if (script) return createPluginVideoTask(requestConfig, selectedModel, script, guidedPrompt, references, options);
    assertVideoConfig(requestConfig, requestConfig.model);
    return createOpenAIVideoTask(requestConfig, selectedModel, guidedPrompt, references, options);
}
