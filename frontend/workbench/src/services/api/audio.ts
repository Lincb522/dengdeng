...
export async function requestAudioGeneration(config: AiConfig, prompt: string, options?: RequestOptions): Promise<Blob> {
    const guidedConfig = applyCreationGuidance(config, "audio");
    const requestConfig = resolveModelRequestConfig(guidedConfig, guidedConfig.model || guidedConfig.audioModel);
    const model = requestConfig.model.trim();
    const format = normalizeAudioFormatValue(config.audioFormat);
    const script = resolveModelScript(guidedConfig, guidedConfig.model || guidedConfig.audioModel);
    const instructions = [config.audioInstructions.trim(), requestConfig.systemPrompt.trim()].filter(Boolean).join("\n\n");
    if (script) {
