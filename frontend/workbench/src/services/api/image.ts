...
import { useDengDengSession } from "@/dengdeng/session";
import { applyCreationGuidance } from "@/creation-library/apply";
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    const guidedConfig = applyCreationGuidance(config, "image");
    const requestConfig = resolveModelRequestConfig(guidedConfig, guidedConfig.model || guidedConfig.imageModel);
    const n = Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1)));
    const script = resolveModelScript(guidedConfig, guidedConfig.model || guidedConfig.imageModel);
    if (script) {
    ...
    try {
        ...
        window.setTimeout(() => void useDengDengSession.getState().refreshUsage(), 800);
        return images;
    } catch (error) {
        throw new Error(readAxiosError(error, apiText("requestFailed")));
    }
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    const guidedConfig = applyCreationGuidance(config, "text");
    const requestConfig = resolveModelRequestConfig(guidedConfig, guidedConfig.model || guidedConfig.textModel);
    const script = resolveModelScript(guidedConfig, guidedConfig.model || guidedConfig.textModel);
    if (script) {
        try {
