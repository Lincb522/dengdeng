...
export function PromptSelectDialog({ open, onOpenChange, onSelect }: { open: boolean; onOpenChange: (open: boolean) => void; onSelect: (prompt: string) => void }) {
    ...
    return (
        <Modal title={t("prompts.library")} open={open} onCancel={() => onOpenChange(false)} footer={null} width={960} centered>
            <div className="grid h-[68dvh] min-h-0 gap-4 md:grid-cols-[190px_minmax(0,1fr)]" data-canvas-no-zoom onWheelCapture={(event) => event.stopPropagation()}>
                <aside className="thin-scrollbar max-h-32 min-h-0 overflow-x-hidden overflow-y-auto border-b border-stone-200 pb-3 dark:border-stone-800 md:max-h-none md:border-b-0 md:border-r md:pb-0 md:pr-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {promptCategories.map((category) => (
                            <Tag.CheckableTag
                                ...
                                className={cn("prompt-filter-tag max-w-full truncate align-middle", selectedCategory === category && "is-active")}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {promptTags.map((tag) => {
                            ...
                            return (
                                <Tag.CheckableTag key={tag} checked={active} className={cn("prompt-filter-tag max-w-full truncate align-middle", active && "is-active")} onChange={() => toggleTag(tag)}>
