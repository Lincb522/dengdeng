...
export function PromptCard({
    ...
    const previewClassName = compact ? "aspect-[4/3] w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" : "aspect-[4/3] w-full object-cover";
    const placeholderClassName = "grid aspect-[4/3] w-full place-items-center bg-stone-100 text-stone-400 dark:bg-stone-900 dark:text-stone-600";
        ...
        <Card
            ...
            cover={
                <button type="button" className="block w-full cursor-pointer text-left" onClick={onOpen}>
                    {item.coverUrl && !previewFailed ? (
                        <img src={item.coverUrl} alt={item.title} className={previewClassName} loading="lazy" referrerPolicy="no-referrer" onError={() => setPreviewFailed(true)} />
                    ) : (
        ...
        >
            <button type="button" className={compact ? "block w-full cursor-pointer text-left" : "block w-full flex-1 cursor-pointer text-left"} onClick={onOpen}>
                <div className={compact ? "px-3 py-2.5" : "p-4"}>
                    ...
                    {compact ? (
                        <p className="mt-1 truncate text-[11px] text-stone-500 dark:text-stone-400">{item.category}</p>
                    ) : (
                        <>
