...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between"><h1 class="text-xl font-bold text-slate-100">模型配置</h1><button class="btn-primary" @click="create">新增模型</button></div>
    ...
    <Teleport to="body"><div v-if="showForm" class="legacy-modal-backdrop fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" @click.self="showForm = false"><div class="card w-full max-w-lg p-6"><h3 class="mb-5 text-base font-semibold text-slate-100">{{ editing ? '编辑模型配置' : '新增模型配置' }}</h3><div class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4"><div><label class="label">平台</label><ProviderSelect v-model="form.platform" aria-label="模型平台" @update:model-value="form.image_group_id = 0" /></div><div><label class="label">类型</label><select v-model="form.kind" class="input" @change="form.kind !== 'image' && (form.image_group_id = 0)"><option value="chat">对话</option><option value="image">生图 / 编辑</option></select></div></div>
