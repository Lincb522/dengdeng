...
import { defaultSortDirection, sortDirectionOptions, type SortMeaning } from '../../api/sorting'
...
const usageSortMeaning = computed<SortMeaning>(() => filters.value.sort === 'created_at' ? 'time' : 'number')
const usageOrderOptions = computed(() => sortDirectionOptions(usageSortMeaning.value))
...
function changeUsageSort() {
	filters.value.order = defaultSortDirection(usageSortMeaning.value)
	search()
}
  ...
  <div>
    ...
    <section class="usage-filters card mb-4 p-3">
      ...
      <div v-if="expanded" class="usage-filter-grid usage-filter-grid--advanced">
		...
		<label><span>排序</span><select v-model="filters.sort" class="input" @change="changeUsageSort"><option value="created_at">调用时间</option><option value="cost_micro">费用</option><option value="first_token_ms">首字耗时</option><option value="duration_ms">总耗时</option><option value="status_code">状态码</option></select></label>
		<label><span>顺序</span><select v-model="filters.order" class="input" @change="search"><option v-for="option in usageOrderOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>
      </div>
    </section>
