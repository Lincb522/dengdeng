    ...
    <template v-if="tab === 'alerts'">
      <section class="card mb-5 overflow-x-auto">
        ...
        <table v-responsive-table class="table-base min-w-[760px]"><thead><tr><th>规则</th><th>触发条件</th><th>范围</th><th>通知</th><th>状态</th><th class="text-right">操作</th></tr></thead>
          <tbody><tr v-for="rule in rules" :key="rule.id"><td class="font-medium text-slate-200">{{ rule.name }}</td><td><span class="tag-gray">{{ rule.metric_type ? `${rule.metric_type} ${rule.operator} ${rule.threshold}` : rule.condition === 'down' ? '仅不可用' : rule.condition === 'degraded_or_down' ? '降级或不可用' : '非健康状态' }}</span><div v-if="rule.metric_type" class="mt-1 text-[10px] text-slate-500">{{ rule.window_minutes }} 分钟窗口 · {{ rule.cooldown_minutes }} 分钟冷却</div></td><td class="text-xs text-slate-400"><span v-if="rule.platform" class="provider-inline-label"><ProviderLogo :platform="rule.platform" size="sm" />{{ rule.account_id ? `账号 #${rule.account_id}` : rule.group_id ? `分组 #${rule.group_id}` : (PLATFORM_LABELS[rule.platform] || rule.platform) }}</span><span v-else>{{ rule.account_id ? `账号 #${rule.account_id}` : rule.group_id ? `分组 #${rule.group_id}` : '全部账号' }}</span></td><td class="text-xs text-slate-500">{{ rule.notify_email || '管理邮箱 / 控制台' }}</td><td><span :class="rule.enabled ? 'tag-green' : 'tag-gray'">{{ rule.enabled ? '启用' : '暂停' }}</span></td><td class="space-x-2 text-right"><button class="btn-ghost !px-2.5 !py-1 text-xs" @click="toggleRule(rule)">{{ rule.enabled ? '暂停' : '启用' }}</button><button class="btn-ghost !px-2.5 !py-1 text-xs" @click="openEdit(rule)">编辑</button><button class="btn-danger !px-2.5 !py-1 text-xs" @click="removeRule(rule)">删除</button></td></tr><tr v-if="!rules.length"><td colspan="6" class="py-10 text-center text-sm text-slate-500">还没有规则。默认的“上游账号不可用”规则会在服务启动后自动创建。</td></tr></tbody>
        </table>
      </section>
